...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withWriter: uq.withWriter.Clone(),
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withWriter; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Writer = []*Book{}
		}
		query.withFKs = true
		query.Where(predicate.Book(func(s *sql.Selector) {
			s.Where(sql.InValues(user.WriterColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_writer
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_writer" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_writer" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Writer = append(node.Edges.Writer, n)
		}
	}
